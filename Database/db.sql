...
CREATE TABLE credit_card_detail(
    customer_id char(7),
	credit_card_number char(16) check(LEN(credit_card_number)=16),
	cvv char(3) CHECK(LEN(cvv)=3),
	expire_year NUMERIC(4,0) check(expire_year>1950 and expire_year<2100),
    expire_month numeric(2,0) check(expire_month>0 and expire_month<13),
...
CREATE TABLE varient(
    ...
    restock_limit int CHECK(restock_limit>=0),
...
CREATE TABLE `order`(
	order_id char(7),
	state char(30) check (state in ('Pending', 'Deliverying', 'Delivered','Waiting for Customer')),
	delivery_method char(20) check (delivery_method in ('Store Pickup', 'Delivered')),
	payment_method char(25) check (payment_method in ('Credit Card','Cash on Delivery')),
	credit_card_number char(16) check(LEN(credit_card_number)=16),
    cvv char(3) CHECK(LEN(cvv)=3),
	expire_year NUMERIC(4,0) check(expire_year>1950 and expire_year<2100),
    expire_month numeric(2,0) check(expire_month>0 and expire_month<13),
...
CREATE TABLE varient_item(
    ...
    FOREIGN KEY (product_id,varient_id) REFERENCES varient(product_id,varient_id)
    ON UPDATE CASCADE ON DELETE RESTRICT
);
...
CREATE TABLE product_category_specialized_attribute(
    ...
    FOREIGN KEY (product_id) REFERENCES product(product_id)
    ON UPDATE CASCADE ON DELETE RESTRICT,
    FOREIGN KEY (attribute_id) REFERENCES category_specialized_attribute(attribute_id)
    ON UPDATE CASCADE ON DELETE RESTRICT,
    FOREIGN KEY(category_id) REFERENCES category(category_id)
    ON UPDATE CASCADE ON DELETE RESTRICT
);
...
CREATE TABLE delivery_detail(
	...
	delivery_person_id char(5),
	address varchar(255) not null,
	estimate_days numeric(2,0) check (estimate_days >=0),
